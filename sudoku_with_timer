...
mutex consoleMutex;
atomic<bool> timeUp(false); // Flag to signal when the countdown ends
...
void showCountdownClock(int seconds);
...
void playSudoku()
{
    ...
    else
    {
        int countDownTime = 5;
        timeUp = false;
        thread countdownThread(showCountdownClock, countDownTime);
        while(!timeUp)
        {
...
char gameLevel(){
    ...
    do{
        ...
        gamePart("line_small_right");
        colorSet(13);
        cout << setw(29) << "          SELECT A DIFFICULTY" << endl;
        colorSet(4);
        gamePart("seperator");
        colorSet(15);
        cout << "[1]";
...
void mainGame()
{
    ...
    case 1:
        colorSet(10);
        cout << setw(22) << "            EASY LEVEL" << endl;
        break;
    case 2:
        colorSet(14);
        cout << setw(23) << "            MEDIUM LEVEL" << endl;
        break;
    default:
        colorSet(12);
        cout << setw(22) << "            HARD LEVEL" << endl;
    }
    colorSet(4);
    gamePart("seperator");
    cout << endl;
...
void exitMenu()
{
    do{
        ...
        gamePart("line_small_right");
        colorSet(13);
        cout << setw(28) << "        DO YOU WANT TO EXIT?" << endl;
        colorSet(4);
        gamePart("seperator");
        colorSet(15);
        cout << "[1]";
...
void showCountdownClock(int seconds) {
    const int clockX = 0; // Column position of the clock
    const int clockY = 0; // Row position of the clock
    ...
    while (seconds >= 0 && !timeUp) {
        ...
        COORD savedPosition = getCursorPosition();
            // Move the cursor to the clock position
            moveCursorTo(clockX, clockY);

            // Display the countdown clock
            cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
            colorSet(12);
            cout << "Time: " << setw(2)  << setfill('0') << minutes << ":"
                      << setw(2) << setfill('0') << remainingSeconds << "   "; // Extra spaces to clear old text

            // Restore the cursor position
            moveCursorTo(savedPosition.X, savedPosition.Y);
    ...
    if (!timeUp) {
            ...
            // Move the cursor to the clock position and display "Time's up!"
            //moveCursorTo(clockX, clockY);
            sudokuStatus = "quitted";
            cout << "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\nTime's up!    "; // Clear old clock text
            if (countdownThread.joinable()) {
                countdownThread.join();
            }
            mainGame();
